...
프로필 설문과 여행 설문은 별개입니다. 자동 일정 생성에는 여행별 성향이 필요하므로 기존 9문항 설문 UI를 재사용해 해당 여행으로 제출합니다. 개인 경로는 로그인한 본인의 userId만 사용합니다.

## 지원 범위

기존 서버 API만 사용합니다. 개인 출발·귀가 장소 변경 UI와 API 호출은 제공하지 않습니다. 개인 경로는 이미 등록된 장소를 사용하며 장소가 없으면 추천할 수 없다는 안내를 표시합니다.

자동 일정과 ITINERARY 경로는 dev에서 검증했습니다. 개인 DEPARTURE/HOME 경로의 실서버 검증은 이번 범위에 포함하지 않습니다.

## 검증

- Android JVM 테스트 198개, 실패/오류 0.
...
dev 빌드 및 로그아웃된 전용 에뮬레이터에서만 실행합니다. 토큰/계정 비밀번호는 출력하지 않습니다.
